...
async function deleteIncome(req, res) {
  ...
  res.json({ message: "Income Deleted", status: res.statusCode });
}
const weekday = [
  "Sunday",
  "Monday",
...
async function addIncome(req, res) {
  ...
  res.json({ message: "Income Added", data: inc, status: res.statusCode });
}
...
async function addExpense(req, res) {
  ...
  res.json({ message: "Expense Added", data: exp, status: res.statusCode });
}
...
async function deleteExpense(req, res) {
  ...
  res.json({ message: "Expense Deleted", status: res.statusCode });
}
...
async function register(req, res) {
  try {
    ...
    if (!(email && pwd && first_name && last_name)) {
      res
        .status(400)
        .json({ message: "All input is required", status: res.statusCode });
    }
...
async function login(req, res) {
  try {
    ...
    res
      .status(400)
      .json({ message: "Invalid email or password", status: res.statusCode });
  } catch (err) {
    console.log(err);
  }
}
