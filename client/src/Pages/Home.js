...
// Adding API
import {
  getAllIncomesInYearByUserId,
  getAllIncomesInYearWithTypesByUserId,
} from "../service/IncomeService";
import {
  getAllExpensesInYearByUserId,
  getAllExpenseInYearWithTypesByUserId,
...
function Home() {
  ...
  const monthsDropdown = months.map((item_month, index) => {
    return (
      <Dropdown.Item
        key={index}
        onClick={(e) => changeMonth(e, item_month.value, item_month.name)}
      >
        {item_month.name}
      </Dropdown.Item>
    );
  ...
  async function changeMonth(e, month_value, month_name) {
    ...
    if (token) {
      ...
      try {
        ...
        const responseIncomeMonth = await getAllIncomesInYearWithTypesByUserId(
          month_value,
          user_id
        );
        const responseExpenseMonth = await getAllExpenseInYearWithTypesByUserId(
          month_value,
          user_id
        );
  ...
  async function getMonthData() {
    ...
    if (token) {
      ...
      try {
        ...
        // Update arrayExpense
        const newArrayExpense = Object.values(
          responseExpenseMonth.data.sumType
        );
        ...
        <div>
            ...
            <Col md={2}>
              <div className="HeaderChart">
                <Dropdown>
                  ...
                  <Dropdown.Menu>{monthsDropdown}</Dropdown.Menu>
                </Dropdown>
              </div>
            </Col>
          </Row>
          <Row className="BgBox">
            ...
            <Col>
              <Row style={{ padding: "10px" }}>
                <MoneyCardBlue
                  amountMoney={incomeMonth}
                  cardText={"Total Income"}
                />
                <MoneyCardRed
                  amountMoney={expenseMonth}
                  cardText={"Total Expense"}
                />
                <MoneyCardGreen
                  amountMoney={totalMonth}
                  cardText={"Total Summary"}
                />
              </Row>
            </Col>
          ...
          >
            <Container fluid style={{ backgroundColor: "whitesmoke" }}>
              {!loadingArray ? (
                <BarChartWeek nestedArray={nestedArray} />
              ) : (
                <div>LOADING</div>
              )}
            </Container>
